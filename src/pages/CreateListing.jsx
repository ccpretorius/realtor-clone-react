...
export default function CreateListing() {
  ...
  const { type, name, bedrooms, bathrooms, parking, address, furnished, description, offer, regularPrice, discountedPrice, latitude, longitude, images } = formData;
  ...
  function onChange(e) {
    ...
    if (!e.target.files) {
      ...
      })); // boolean ?? means if boolean is null it sets the value to the text or number in the e.target.value. Otherwise it will render the boolean value (for those yes/no buttons)
    }
  }
  ...
  async function onSubmit(e) {
    ...
    if (+discountedPrice >= +regularPrice) {
      setLoading(false);
      toast.error("Regular price must be higher than the discounted price");
      return;
    }
    if (images.length > 6) {
      setLoading(false);
